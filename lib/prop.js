...
const baseProp = (rawOptions, Type, target, key, isArray = false) => {
    ...
    if (ref) {
        data_1.schema[name][key] = Object.assign({}, data_1.schema[name][key], { type: mongoose.Schema.Types.ObjectId, ref: ref.modelName ? ref.modelName : ref.name });
        return;
    }
    const itemsRef = rawOptions.itemsRef;
    if (itemsRef) {
        data_1.schema[name][key][0] = Object.assign({}, data_1.schema[name][key][0], { type: mongoose.Schema.Types.ObjectId, ref: itemsRef.modelName ? itemsRef.modelName : itemsRef.name });
        return;
    }
